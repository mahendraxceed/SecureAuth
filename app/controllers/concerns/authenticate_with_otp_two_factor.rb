module AuthenticateWithOtpTwoFactor
  ...
  def user_params
    params.require(:user).permit(:login, :email, :mobile_no, :password, :remember_me, :otp_attempt)
  end
  ...
  def find_user
    ...
    elsif user_params[:mobile_no]
      User.find_by(mobile_no: user_params[:mobile_no])
    end
  end
